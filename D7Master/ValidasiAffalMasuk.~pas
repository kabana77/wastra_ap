...
type
  TValidasiAffalMasukFrm = class(TForm)
    Panel1: TPanel;
    ...
    wwDBGrid6: TwwDBGrid;
    QDocBC: TOracleDataSet;
    dsQDocBC: TwwDataSource;
    QDocBCNO_BC: TStringField;
    QDocBCTANGGAL_BC: TDateTimeField;
    QDocBCNO_NOTA: TStringField;
    QDocBCNO_REG_PEMAKAIAN: TFloatField;
    QDocBCKD_ITEM: TStringField;
    QDocBCNAMA_ITEM: TStringField;
    QDocBCSATUAN: TStringField;
    QDocBCQTY: TFloatField;
    QDocBCVALUE: TFloatField;
    LookWaste: TwwDBLookupComboDlg;
    QLookWaste: TOracleDataSet;
    QLookWasteNO_NOTA: TStringField;
    QLookWasteTANGGAL: TDateTimeField;
    QLookWasteKD_ITEM: TStringField;
    QLookWasteKETERANGAN: TStringField;
    QLookWasteQTY: TFloatField;
    QLookWasteSATUAN: TStringField;
    QLookWasteNO_LOT: TStringField;
    QLookWasteNO_REG_PEMAKAIAN: TIntegerField;
...
var
  ValidasiAffalMasukFrm: TValidasiAffalMasukFrm;
...
procedure TValidasiAffalMasukFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
  ValidasiAffalMasukFrm:=Nil;
end;

procedure TValidasiAffalMasukFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TValidasiAffalMasukFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TValidasiAffalMasukFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TValidasiAffalMasukFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TValidasiAffalMasukFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
...
procedure TValidasiAffalMasukFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TValidasiAffalMasukFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TValidasiAffalMasukFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TValidasiAffalMasukFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TValidasiAffalMasukFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TValidasiAffalMasukFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TValidasiAffalMasukFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TValidasiAffalMasukFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TValidasiAffalMasukFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TValidasiAffalMasukFrm.ECariChange(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TValidasiAffalMasukFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TValidasiAffalMasukFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;

  if DMFrm.QUserID_HAK.AsString = 'B' then
  begin
...
procedure TValidasiAffalMasukFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TValidasiAffalMasukFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TValidasiAffalMasukFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TValidasiAffalMasukFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
  begin
...
procedure TValidasiAffalMasukFrm.FormCloseQuery(Sender: TObject;
...
procedure TValidasiAffalMasukFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TValidasiAffalMasukFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TValidasiAffalMasukFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TValidasiAffalMasukFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TValidasiAffalMasukFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TValidasiAffalMasukFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TValidasiAffalMasukFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TValidasiAffalMasukFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TValidasiAffalMasukFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TValidasiAffalMasukFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TValidasiAffalMasukFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TValidasiAffalMasukFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TValidasiAffalMasukFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TValidasiAffalMasukFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TValidasiAffalMasukFrm.QLookLokasiBeforeOpen(DataSet: TDataSet);
begin
  QLookLokasi.SetVariable('kd_jns_item',QDetailKD_JNS_ITEM.AsString);
end;

procedure TValidasiAffalMasukFrm.LookLokasiCloseUp(Sender: TObject;
...
procedure TValidasiAffalMasukFrm.LookLokasiEnter(Sender: TObject);
begin
  QLookLokasi.Close;
  QLookLokasi.Open;
end;

procedure TValidasiAffalMasukFrm.BtnFormulasiClick(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.LookKoreksiEnter(Sender: TObject);
begin
  QJnsKoreksi.Open;
end;

procedure TValidasiAffalMasukFrm.LookSuplierCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
var
  vkota : string;
begin
...
procedure TValidasiAffalMasukFrm.LookSuplierEnter(Sender: TObject);
begin
  DMFrm.QSuplier.Open;
end;

procedure TValidasiAffalMasukFrm.QDetailCalcFields(DataSet: TDataSet);
begin
...
procedure TValidasiAffalMasukFrm.wwDBGrid2UpdateFooter(Sender: TObject);
begin
  QSubTotal.Close;
...
procedure TValidasiAffalMasukFrm.QBrowseCalcFields(DataSet: TDataSet);
begin
  QBrowseQTY_BALE.AsFloat:=QBrowseQTY.AsFloat/cball2kg;
end;

procedure TValidasiAffalMasukFrm.LookItemEnter(Sender: TObject);
begin
  QLookItem.close;
  QLookItem.Open;
end;

procedure TValidasiAffalMasukFrm.QLookItemBeforeOpen(DataSet: TDataSet);
var
  vdd,vmm,vyy : word;
  vtgl_awal : TDate;
begin
...
procedure TValidasiAffalMasukFrm.LookItemCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
...
procedure TValidasiAffalMasukFrm.QDetailLOKASIChange(Sender: TField);
begin
if QDetailMESIN.AsString <> '-' then
 begin
    QDetailQTY_KG_PALET.AsFloat:=(QDetailQTY.AsFloat-QDetailQTY_BALL.AsFloat);
...
procedure TValidasiAffalMasukFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
        ...
        begin
          ...
          begin
            ...
            begin
              if vpertama then
                begin
                  vfilter:=vfilter+QMaster.Fields[i].FieldName+' like ''%'+Edit1.Text+'%''';
                  vpertama:=False;
                end
                else
                  vfilter:=vfilter+' or '+QMaster.Fields[i].FieldName+' like ''%'+Edit1.Text+'%''';
            end;
          end;
          vorder:=' order by no_nota desc';
          //vorder:=' order by '+wwDBGrid1.Columns[0].FieldName;
        end;
        ShowMessage(QMaster.SQL.Text+vfilter+vorder);
...
procedure TValidasiAffalMasukFrm.BtnEditingClick(Sender: TObject);
begin
...
procedure TValidasiAffalMasukFrm.BtnBrowseClick(Sender: TObject);
begin
{  if BtnSimpan.Enabled then
...
procedure TValidasiAffalMasukFrm.QMasterAfterDelete(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TValidasiAffalMasukFrm.LookLOTEnter(Sender: TObject);
begin
  DMFrm.QNoLot.Close;
  DMFrm.QNoLot.Open;
end;

procedure TValidasiAffalMasukFrm.QMasterAfterPost(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TValidasiAffalMasukFrm.QMasterAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data'; 
end;

procedure TValidasiAffalMasukFrm.BitBtn9Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
  ...
  if cbTanggalinv.Checked then
    vfilter:=' where (tanggal_bc>=to_date('''+FormatDateTime('dd/mm/yyyy',VTglAwinv.Date)+''',''dd/mm/yyyy'') and tanggal_bc<=(to_date('''+FormatDateTime('dd/mm/yyyy',VTglAkinv.Date)+''',''dd/mm/yyyy'')+1-1/86400)) and ('
  else
    vfilter:=' where (';
  if (QDocBC.FieldCount>=1) then
  begin
    if dbcFieldinv.ItemIndex=0 then
    begin
      for i:=0 to wwDBGrid6.Selected.Count-1 do
      begin
        if (QDocBC.FieldByName(wwDBGrid6.Columns[i].FieldName).FieldKind=fkData) and (UpperCase(wwDBGrid6.Columns[i].FieldName)<>'TANGGAL_BC') then
        begin
          if vpertama then
            begin
    ...
    vfilter:=vfilter+')';
    if QDocBC.Active then
      vorder:=' order by '+wwDBGrid6.Columns[0].FieldName
      else
        vorder:=' order by no_nota desc';

  end;
  QDocBC.DisableControls;
  QDocBC.Close;
  QDocBC.SetVariable('myparam',vfilter+vorder);
  QDocBC.Open;
  QDocBC.EnableControls;
{azmi}

end;

procedure TValidasiAffalMasukFrm.TabSheet3Show(Sender: TObject);
begin
  QDocBC.Open;
end;

procedure TValidasiAffalMasukFrm.vTglAwinvChange(Sender: TObject);
begin
  vTglAkinv.Date:=Trunc(EndOfTheMonth(VTglAwinv.Date));
end;

procedure TValidasiAffalMasukFrm.LookWasteCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  QDocBCNO_NOTA.AsString:=QLookWasteNO_NOTA.AsString;

  if QLookWasteNO_LOT.AsString<>'' then
    QDocBCKD_ITEM.AsString:=QLookWasteKD_ITEM.AsString+'_'+QLookWasteNO_LOT.AsString
  else
    QDocBCKD_ITEM.AsString:=QLookWasteKD_ITEM.AsString;

  QDocBCNO_REG_PEMAKAIAN.AsInteger:=QLookWasteNO_REG_PEMAKAIAN.AsInteger;
  QDocBCNAMA_ITEM.AsString:=QLookWasteKETERANGAN.AsString;
  QDocBCSATUAN.AsString:=QLookWasteSATUAN.asstring;
  QDocBCQTY.AsFloat:=QLookWasteQTY.AsFloat;
end;

procedure TValidasiAffalMasukFrm.LookWasteEnter(Sender: TObject);
begin
  QLookWaste.Close;
  QLookWaste.Open;
end;
