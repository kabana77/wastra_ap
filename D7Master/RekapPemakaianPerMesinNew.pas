unit RekapPemakaianPerMesinNew;
...
type
  TRekapPemakaianPerMesinNewFrm = class(TForm)
    Panel1: TPanel;
    ...
    QBrowseSP: TStringField;
    QLookNilaiHARGA: TFloatField;
    QBrowseHARGA: TIntegerField;
    QBrowseHARGA2: TIntegerField;
...
var
  RekapPemakaianPerMesinNewFrm: TRekapPemakaianPerMesinNewFrm;

implementation

uses DM, MainBrowse, Main, Menus, XPROCS, DateUtils, Keterangan, Sisdur,
  {RekapPemakaianPerMesin2BB,} {RekapPemakaianPerMesin2KIM,}
  RekapPemakaianPerMesin2;

{$R *.dfm}

procedure TRekapPemakaianPerMesinNewFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
  RekapPemakaianPerMesinNewFrm:=Nil;
end;

procedure TRekapPemakaianPerMesinNewFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QLookSubBagian.Close;
  QLookItem.Close;
end;

procedure TRekapPemakaianPerMesinNewFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TRekapPemakaianPerMesinNewFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TRekapPemakaianPerMesinNewFrm.QLookItemBeforeOpen(DataSet: TDataSet);
begin
  QLookItem.SetVariable('kd_jns_kebutuhan',vkd_jns_kebutuhan);
end;

procedure TRekapPemakaianPerMesinNewFrm.LookItemCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TRekapPemakaianPerMesinNewFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TRekapPemakaianPerMesinNewFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
  vdd, vmm, vyy : word;
...
begin
  ...
  QProses.Close;
  QProses.SetVariable('kd_jns_item',DMFrm.QJns_ItemKD_JNS_ITEM.AsString);
...
{  QLookNilai.SetVariable('BXX','M'+FormatFloat('00',vmm)+'/decode(A'+FormatFloat('00',vmm)+',0,1,A'+FormatFloat('00',vmm)+') as harga');
  ...
  //    for i:=0 to QBrowse.FieldCount-1 do
      begin
       { QLookNilai.Close;
        QLookNilai.SetVariable('pawal',VTglAwal.Date);
        QLookNilai.SetVariable('pakhir',VTglAkhir.Date);
        QLookNilai.SetVariable('no_nota',QBrowseNO_NOTA.AsString);
        QLookNilai.SetVariable('kd_item',QBrowseKD_ITEM.AsString);
        QLookNilai.Open; }
...
//    vorder:=' order by '+wwDBGrid1.Columns[0].FieldName;
    if QBrowse.Active then
      vorder:=' order by '+wwDBGrid1.Columns[0].FieldName
        ...
        vorder:=' order by tanggal, kd_item';
...
procedure TRekapPemakaianPerMesinNewFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
end;

procedure TRekapPemakaianPerMesinNewFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
//  QBrowse.SetVariable('kd_transaksi','%');
end;

procedure TRekapPemakaianPerMesinNewFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=Caption;
end;

procedure TRekapPemakaianPerMesinNewFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.FormCloseQuery(Sender: TObject;
...
procedure TRekapPemakaianPerMesinNewFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TRekapPemakaianPerMesinNewFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TRekapPemakaianPerMesinNewFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TRekapPemakaianPerMesinNewFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TRekapPemakaianPerMesinNewFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TRekapPemakaianPerMesinNewFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TRekapPemakaianPerMesinNewFrm.BtnExportClick(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TRekapPemakaianPerMesinNewFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TRekapPemakaianPerMesinNewFrm.LookItemUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TRekapPemakaianPerMesinNewFrm.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Inc(vnomer);
end;

procedure TRekapPemakaianPerMesinNewFrm.QBrowseCalcFields(DataSet: TDataSet);
begin
  if Copy(QBrowseKD_ITEM.AsString,1,1)='1' then
    //QBrowseHARGA2.AsFloat:=QBrowseHARGA.AsFloat*181.44
    QBrowseHARGA2.AsInteger:=QBrowseHARGA.AsInteger
    else
    QBrowseHARGA2.AsInteger:=QBrowseHARGA.AsInteger;
  QBrowseSUB_TOTAL.AsFloat:=QBrowseHARGA2.AsInteger*QBrowseQTY_PP.AsInteger;
end;

procedure TRekapPemakaianPerMesinNewFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TRekapPemakaianPerMesinNewFrm.wwDBGrid1TitleButtonClick(
...
procedure TRekapPemakaianPerMesinNewFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.cbAllClick(Sender: TObject);
begin
  QDetail2.DisableControls;
  QDetail2.Close;
  QDetail2.SetVariable('vjns',DMFrm.QJns_ItemKD_JNS_ITEM.AsString);
...
procedure TRekapPemakaianPerMesinNewFrm.BitBtn2Click(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.wwDBLookupCombo1Change(
  Sender: TObject);
begin
  Caption:='Rekap Nilai Pemakaian Per Masin '+wwDBLookupCombo1.Text;
end;

procedure TRekapPemakaianPerMesinNewFrm.BitBtn3Click(Sender: TObject);
begin
...
procedure TRekapPemakaianPerMesinNewFrm.TabSheet3Show(Sender: TObject);
//var
 // vtot : integer;
begin
  QBrowse.Close;
...
procedure TRekapPemakaianPerMesinNewFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TRekapPemakaianPerMesinNewFrm.QDetail2CalcFields(
  DataSet: TDataSet);
begin
