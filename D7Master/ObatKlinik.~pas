...
uses
  ...
  Wwdbgrid, StdCtrls, Buttons, ComCtrls, ExtCtrls, wwDBNavigator, wwrcdvw;

type
  TObatKlinikFrm = class(TForm)
    Panel1: TPanel;
...
procedure TObatKlinikFrm.FormClose(Sender: TObject;
...
procedure TObatKlinikFrm.BtnSimpanClick(Sender: TObject);
begin
...
procedure TObatKlinikFrm.QMasterAfterPost(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TObatKlinikFrm.BtnEditingClick(Sender: TObject);
begin
...
procedure TObatKlinikFrm.BtnBrowseClick(Sender: TObject);
begin
...
procedure TObatKlinikFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TObatKlinikFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QSatuan.Close;
...
procedure TObatKlinikFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TObatKlinikFrm.QMasterAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TObatKlinikFrm.BitBtn5Click(Sender: TObject);
var
  vtitle : String;
begin
  case PageControl1.ActivePageIndex of
  0 : begin
        vtitle:='Daftar Obat Klinik';
...
procedure TObatKlinikFrm.TabSheet1Show(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TObatKlinikFrm.BtnEditing2Click(Sender: TObject);
begin
...
procedure TObatKlinikFrm.BtnBrowse2Click(Sender: TObject);
begin
  if BtnSimpan.Enabled then
    ShowMessage('Simpan/ Posting Data Dulu !')
    else
...
procedure TObatKlinikFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TObatKlinikFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TObatKlinikFrm.TabSheet2Show(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TObatKlinikFrm.FormShow(Sender: TObject);
begin
  Height:=600;
  Width:=716;
end;

procedure TObatKlinikFrm.LookMesinEnter(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TObatKlinikFrm.LookJenisEnter(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TObatKlinikFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TObatKlinikFrm.QMasterNewRecord(DataSet: TDataSet);
begin
  QMasterKD_JNS_ITEM.AsString:='7';
  QMasterKD_SATUAN.AsString:='01';
  QMasterKD_KEL.AsString:='01';
end;

procedure TObatKlinikFrm.QMaster2NewRecord(DataSet: TDataSet);
begin
  QMaster2KD_JNS_ITEM.AsString:='7';
end;

procedure TObatKlinikFrm.FormCloseQuery(Sender: TObject;
...
procedure TObatKlinikFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TObatKlinikFrm.wwDBGrid1CalcCellColors(Sender: TObject;
...
procedure TObatKlinikFrm.LookSatuanEnter(Sender: TObject);
begin
  DMFrm.QSatuan.Open;
end;

procedure TObatKlinikFrm.cbOtomatisClick(Sender: TObject);
begin
...
procedure TObatKlinikFrm.ECariChange(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TObatKlinikFrm.LTitleClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
