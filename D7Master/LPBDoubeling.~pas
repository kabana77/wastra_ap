...
type
  TLPBDoubelingFrm = class(TForm)
    ...
    BitBtn3: TBitBtn;
    QDetailBALE: TFloatField;
    QDetailQTY3: TFloatField;
    QDetailQTY4: TFloatField;
    QDetailQTY_WASTE: TFloatField;
    QDetailQTY_WASTE2: TFloatField;
    QBrowseQTY_LPB: TFloatField;
    QBrowseQTY_LPB2: TFloatField;
    QBrowseQTY_WASTE: TFloatField;
    QBrowseQTY_WASTE2: TFloatField;
    QTotalQTY2: TFloatField;
    QTotalQTY_LPB: TFloatField;
    QTotalQTY_LPB2: TFloatField;
    QRLabel18: TQRLabel;
    QRLabel22: TQRLabel;
...
var
  LPBDoubelingFrm: TLPBDoubelingFrm;
...
procedure TLPBDoubelingFrm.FormClose(Sender: TObject;
...
procedure TLPBDoubelingFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QLookSubBagian.Close;
  QLookItem.Close;
end;

procedure TLPBDoubelingFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TLPBDoubelingFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TLPBDoubelingFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TLPBDoubelingFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TLPBDoubelingFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
  QLookItem.Close;
...
procedure TLPBDoubelingFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TLPBDoubelingFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.QDetailNewRecord(DataSet: TDataSet);
begin
  QDetailOPR_INSERT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TLPBDoubelingFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TLPBDoubelingFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
 { if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TLPBDoubelingFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TLPBDoubelingFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TLPBDoubelingFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TLPBDoubelingFrm.QLookItemBeforeOpen(DataSet: TDataSet);
var
  vdd,vmm,vyy : word;
  vtgl_awal : TDate;
begin
...
procedure TLPBDoubelingFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TLPBDoubelingFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TLPBDoubelingFrm.ECariChange(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TLPBDoubelingFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
//    vorder:=' order by '+wwDBGrid1.Columns[0].FieldName;
  ...
  wwDBGrid1.ColumnByName('QTY2').FooterValue:=FormatFloat('#,##0.##;(#,##0.##)',QTotalQTY2.AsFloat);
  wwDBGrid1.ColumnByName('QTY_LPB').FooterValue:=FormatFloat('#,##0.######;(#,##0.######); ',QTotalQTY_LPB.AsFloat);
  wwDBGrid1.ColumnByName('QTY_LPB2').FooterValue:=FormatFloat('#,##0.######;(#,##0.######); ',QTotalQTY_LPB2.AsFloat);

end;

procedure TLPBDoubelingFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TLPBDoubelingFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TLPBDoubelingFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TLPBDoubelingFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.QLookSubBagianBeforeOpen(DataSet: TDataSet);
begin
  QLookSubBagian.SetVariable('kd_bag',vkd_bag);
end;

procedure TLPBDoubelingFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TLPBDoubelingFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
...
procedure TLPBDoubelingFrm.FormCloseQuery(Sender: TObject;
...
procedure TLPBDoubelingFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
{  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TLPBDoubelingFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TLPBDoubelingFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TLPBDoubelingFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TLPBDoubelingFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TLPBDoubelingFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TLPBDoubelingFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TLPBDoubelingFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TLPBDoubelingFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TLPBDoubelingFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TLPBDoubelingFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TLPBDoubelingFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TLPBDoubelingFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TLPBDoubelingFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TLPBDoubelingFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TLPBDoubelingFrm.LookItemUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TLPBDoubelingFrm.LookBagianUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TLPBDoubelingFrm.QDetailCalcFields(DataSet: TDataSet);
begin
  QDetailBALE.AsFloat:=QDetailQTY.AsFloat/cball2kg;
end;

procedure TLPBDoubelingFrm.QBrowseCalcFields(DataSet: TDataSet);
begin
  QBrowseQTY2.AsFloat:=QBrowseQTY.AsFloat/cball2kg;
end;

procedure TLPBDoubelingFrm.QTotalBeforeOpen(DataSet: TDataSet);
begin
  QTotal.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TLPBDoubelingFrm.BitBtn3Click(Sender: TObject);
begin
  wwDBGrid2Enter(NIL);
  QDetail.Refresh;
end;
