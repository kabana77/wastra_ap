...
uses
  ...
  Mask, Wwdbspin, Buttons, wwclearbuttongroup, wwradiogroup;

type
  TValidasiBonKlinikFrm = class(TForm)
    Panel1: TPanel;
    ...
    QKaryawan_Garment: TOracleDataSet;
    QKaryawan_GarmentNAMA_KARYAWAN: TStringField;
    QKaryawan_GarmentJNS_KEL: TStringField;
    QKaryawan_GarmentJABATAN: TStringField;
    QKaryawan_GarmentBAGIAN: TStringField;
    QKaryawan_GarmentNIK_LAMA: TStringField;
    wwRG_Kar: TwwRadioGroup;
    ...
    procedure LookKaryawanUserButton2Click(Sender: TObject;
      LookupTable: TDataSet);
    procedure wwRG_KarChange(Sender: TObject);
...
var
  ValidasiBonKlinikFrm: TValidasiBonKlinikFrm;
...
procedure TValidasiBonKlinikFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
  ValidasiBonKlinikFrm:=Nil;
end;

procedure TValidasiBonKlinikFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TValidasiBonKlinikFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TValidasiBonKlinikFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TValidasiBonKlinikFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TValidasiBonKlinikFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
...
procedure TValidasiBonKlinikFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TValidasiBonKlinikFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TValidasiBonKlinikFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TValidasiBonKlinikFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TValidasiBonKlinikFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TValidasiBonKlinikFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TValidasiBonKlinikFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TValidasiBonKlinikFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TValidasiBonKlinikFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TValidasiBonKlinikFrm.ECariChange(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TValidasiBonKlinikFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TValidasiBonKlinikFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TValidasiBonKlinikFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TValidasiBonKlinikFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TValidasiBonKlinikFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TValidasiBonKlinikFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
...
procedure TValidasiBonKlinikFrm.FormCloseQuery(Sender: TObject;
...
procedure TValidasiBonKlinikFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TValidasiBonKlinikFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TValidasiBonKlinikFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TValidasiBonKlinikFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TValidasiBonKlinikFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TValidasiBonKlinikFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TValidasiBonKlinikFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TValidasiBonKlinikFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TValidasiBonKlinikFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TValidasiBonKlinikFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TValidasiBonKlinikFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TValidasiBonKlinikFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TValidasiBonKlinikFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TValidasiBonKlinikFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TValidasiBonKlinikFrm.QLookLokasiBeforeOpen(DataSet: TDataSet);
begin
  QLookLokasi.SetVariable('kd_jns_item',QDetailKD_JNS_ITEM.AsString);
end;

procedure TValidasiBonKlinikFrm.LookLokasiCloseUp(Sender: TObject;
...
procedure TValidasiBonKlinikFrm.LookLokasiEnter(Sender: TObject);
begin
  QLookLokasi.Close;
  QLookLokasi.Open;
end;

procedure TValidasiBonKlinikFrm.BtnFormulasiClick(Sender: TObject);
begin
...
procedure TValidasiBonKlinikFrm.LookKoreksiEnter(Sender: TObject);
begin
  QJnsKoreksi.Open;
end;

procedure TValidasiBonKlinikFrm.LookKaryawanEnter(Sender: TObject);
begin
      QKaryawan.Open;
{azmi}
      QKaryawan_Garment.Open;   
{azmi}
end;

procedure TValidasiBonKlinikFrm.LookKaryawanCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
case wwRG_Kar.ItemIndex of
0 :  BEGIN
  if modified then
  begin
...
1 :  BEGIN
  if modified then
  begin
      QDetailID_BAG.AsString:='S';
      QDetailID_SUB_BAG.AsString:='A';
  end;
  END;
END;
{azmi}
...
procedure TValidasiBonKlinikFrm.LookDiagnosaEnter(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TValidasiBonKlinikFrm.LookKKEnter(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TValidasiBonKlinikFrm.LookItemCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TValidasiBonKlinikFrm.LookItemEnter(Sender: TObject);
var
  vdd,vmm,vyy : word;
  vtgl_awal : TDate;
begin
...
procedure TValidasiBonKlinikFrm.QDetailBeforePost(DataSet: TDataSet);
begin
...
procedure TValidasiBonKlinikFrm.LookKaryawanUserButton1Click(
...
1 :  BEGIN
  if InputQuery('Filter','Cari : ',vcari) then
  begin
       QKaryawan_Garment.Close;
       QKaryawan_Garment.SetVariable('myparam',' where nik like ''%'+vcari+'%'' or nama_karyawan like ''%'+
         vcari+'%'' or bagian like ''%'+
         vcari+'%'' or jabatan like ''%'+
         VCARI+'%'' order by nama_karyawan');
        QKaryawan_Garment.open;
...
procedure TValidasiBonKlinikFrm.LookKaryawanUserButton2Click(
  Sender: TObject; LookupTable: TDataSet);
begin
...
1 :  BEGIN
      QKaryawan_Garment.Close;
      QKaryawan_Garment.SetVariable('myparam',' order by nik_lama');
      QKaryawan_Garment.Open;
     END;
END;
{azmi}
...
procedure TValidasiBonKlinikFrm.wwRG_KarChange(Sender: TObject);
begin
{azmi}
case wwRG_Kar.ItemIndex of
0 : LookKaryawan.LookupTable:=QKaryawan;
1 : LookKaryawan.LookupTable:=QKaryawan_Garment;
end;
{azmi}
end;

procedure TValidasiBonKlinikFrm.LookBagianEnter(Sender: TObject);
begin
  QLookSubBagian.Open;
end;

procedure TValidasiBonKlinikFrm.LookBagianCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
