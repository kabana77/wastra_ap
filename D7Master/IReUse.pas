...
var
  IReUseFrm: TIReUseFrm;

implementation

uses DM, MainBrowse, Main, Menus, ATK;

{$R *.dfm}

procedure TIReUseFrm.FormClose(Sender: TObject;
...
procedure TIReUseFrm.BtnSimpanClick(Sender: TObject);
begin
...
procedure TIReUseFrm.QMasterAfterPost(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TIReUseFrm.BtnEditingClick(Sender: TObject);
begin
...
procedure TIReUseFrm.BtnBrowseClick(Sender: TObject);
begin
...
procedure TIReUseFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TIReUseFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QSatuan.Close;
...
procedure TIReUseFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TIReUseFrm.QMasterAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TIReUseFrm.BitBtn5Click(Sender: TObject);
var
  vtitle : String;
begin
  case PageControl1.ActivePageIndex of
  0 : begin
        vtitle:='Daftar Alat Tulis Kantor';
...
procedure TIReUseFrm.TabSheet1Show(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TIReUseFrm.BtnEditing2Click(Sender: TObject);
begin
...
procedure TIReUseFrm.BtnBrowse2Click(Sender: TObject);
begin
  if BtnSimpan.Enabled then
    ShowMessage('Simpan/ Posting Data Dulu !')
    else
...
procedure TIReUseFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TIReUseFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TIReUseFrm.TabSheet2Show(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TIReUseFrm.FormShow(Sender: TObject);
begin
  Height:=600;
  Width:=716;
end;

procedure TIReUseFrm.LookMesinEnter(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TIReUseFrm.LookJenisEnter(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TIReUseFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TIReUseFrm.QMasterNewRecord(DataSet: TDataSet);
begin
  QMasterKD_JNS_ITEM.AsString:='B';
...
procedure TIReUseFrm.QMaster2NewRecord(DataSet: TDataSet);
begin
  QMaster2KD_JNS_ITEM.AsString:='B';
end;

procedure TIReUseFrm.FormCloseQuery(Sender: TObject;
...
procedure TIReUseFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TIReUseFrm.wwDBGrid1CalcCellColors(Sender: TObject;
...
procedure TIReUseFrm.LookSatuanEnter(Sender: TObject);
begin
  DMFrm.QSatuan.Open;
end;

procedure TIReUseFrm.cbOtomatisClick(Sender: TObject);
begin
...
procedure TIReUseFrm.ECariChange(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TIReUseFrm.LTitleClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TIReUseFrm.QMasterBeforePost(DataSet: TDataSet);
begin
 QMasterOPR_EDIT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TIReUseFrm.BitBtn6Click(Sender: TObject);
begin
{azmi}
...
procedure TIReUseFrm.BitBtn4Click(Sender: TObject);
begin
{azmi}
if QLookUpKelKD_KEL.AsString='' then
begin
...
procedure TIReUseFrm.wwDBLookupCombo1CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
{azmi}
...
procedure TIReUseFrm.wwDBLookupCombo1Enter(Sender: TObject);
begin
{azmi}
QLookUpKel.Open;
{azmi}
...
procedure TIReUseFrm.BitBtn2Click(Sender: TObject);
{azmi}
var
  vuser, vpwd : string;
begin
