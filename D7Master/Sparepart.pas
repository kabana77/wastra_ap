...
uses
  ...
  Oracle, Dateutils, wwdbdatetimepicker;

type
  TSparepartFrm = class(TForm)
    Panel1: TPanel;
    ...
    QStokOpname: TOracleQuery;
    QHak: TOracleDataSet;
    QHakHAK: TFloatField;
    dsQHak: TwwDataSource;
    BitBtn7: TBitBtn;
    QMasterKD_ITEM_LAMA: TStringField;
...
procedure TSparepartFrm.FormClose(Sender: TObject;
...
procedure TSparepartFrm.BtnSimpanClick(Sender: TObject);
begin
...
procedure TSparepartFrm.QMasterAfterPost(DataSet: TDataSet);
begin
  BtnSimpan.Enabled:=True;
end;

procedure TSparepartFrm.BtnEditingClick(Sender: TObject);
begin
...
procedure TSparepartFrm.BtnBrowseClick(Sender: TObject);
begin
...
procedure TSparepartFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TSparepartFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QSatuan.Close;
...
procedure TSparepartFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TSparepartFrm.QMasterAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TSparepartFrm.BitBtn5Click(Sender: TObject);
var
  vtitle : String;
begin
  case PageControl1.ActivePageIndex of
  0 : begin
        vtitle:='Daftar Sparepart';
...
procedure TSparepartFrm.TabSheet1Show(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TSparepartFrm.BtnEditing2Click(Sender: TObject);
begin
...
procedure TSparepartFrm.BtnBrowse2Click(Sender: TObject);
begin
  if BtnSimpan.Enabled then
    ShowMessage('Simpan/ Posting Data Dulu !')
    else
...
procedure TSparepartFrm.BitBtn3Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TSparepartFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TSparepartFrm.TabSheet2Show(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TSparepartFrm.FormShow(Sender: TObject);
begin
  Height:=600;
  Width:=716;
end;

procedure TSparepartFrm.LookMesinEnter(Sender: TObject);
begin
  QMaster.Open;
end;

procedure TSparepartFrm.LookJenisEnter(Sender: TObject);
begin
  QMaster2.Open;
end;

procedure TSparepartFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TSparepartFrm.QMasterNewRecord(DataSet: TDataSet);
begin
  QMasterKD_JNS_ITEM.AsString:='3';
  QMasterKD_SATUAN.AsString:='01';
  QMasterICC.AsString:='0';
  QMasterQty.AsString:='0';
end;

procedure TSparepartFrm.QMaster2NewRecord(DataSet: TDataSet);
begin
  QMaster2KD_JNS_ITEM.AsString:='3';
end;

procedure TSparepartFrm.FormCloseQuery(Sender: TObject;
...
procedure TSparepartFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TSparepartFrm.wwDBGrid1CalcCellColors(Sender: TObject;
...
procedure TSparepartFrm.LookSatuanEnter(Sender: TObject);
begin
  DMFrm.QSatuan.Open;
end;

procedure TSparepartFrm.ECariChange(Sender: TObject);
begin
...
procedure TSparepartFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TSparepartFrm.LTitleClick(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TSparepartFrm.QMasterBeforeEdit(DataSet: TDataSet);
begin
 QMasterOPR_EDIT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TSparepartFrm.QMasterBeforePost(DataSet: TDataSet);
begin
 QMasterOPR_EDIT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TSparepartFrm.BitBtn2Click(Sender: TObject);
begin
{azmi}
  DMFrm.OS.Commit;
  QProses.Close;
  QProses.SetVariable('kd_jns_item','3');
  QProses.SetVariable('pawal',StartOftheMonth(Date));
  QProses.SetVariable('pakhir',Date);
  QProses.Execute;
  Showmessage('MULAI Proses Update Jumlah Stok OPNAME !');
  QStokOpname.Close;
  QStokOpname.SetVariable('kd_jns_item','3');
  QStokOpname.SetVariable('kd_kel',QLookUpKelKD_KEL.AsString);
  QStokOpname.Execute;
  Showmessage('SELESAI Proses Update Jumlah Stok OPNAME !');
{azmi}
end;

procedure TSparepartFrm.wwDBLookupCombo1Enter(Sender: TObject);
begin
{azmi}
QLookUpKel.Open;
{azmi}
...
procedure TSparepartFrm.wwDBLookupCombo1CloseUp(Sender: TObject;
...
procedure TSparepartFrm.BitBtn4Click(Sender: TObject);
begin
{azmi}
...
procedure TSparepartFrm.BitBtn6Click(Sender: TObject);
begin
{azmi}
if QLookUpKelKD_KEL.AsString='' then
begin
...
procedure TSparepartFrm.BitBtn7Click(Sender: TObject);
{azmi}
var
  vuser, vpwd : string;
begin
