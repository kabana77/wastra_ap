...
procedure TNotaReturFrm.FormClose(Sender: TObject;
...
procedure TNotaReturFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TNotaReturFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TNotaReturFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TNotaReturFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TNotaReturFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TNotaReturFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TNotaReturFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
...
procedure TNotaReturFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TNotaReturFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TNotaReturFrm.QDetailNewRecord(DataSet: TDataSet);
begin
  QDetailOPR_INSERT.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TNotaReturFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TNotaReturFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TNotaReturFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TNotaReturFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TNotaReturFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TNotaReturFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TNotaReturFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TNotaReturFrm.ECariChange(Sender: TObject);
begin
...
procedure TNotaReturFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TNotaReturFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TNotaReturFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TNotaReturFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
//  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TNotaReturFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TNotaReturFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TNotaReturFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TNotaReturFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TNotaReturFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TNotaReturFrm.FormCloseQuery(Sender: TObject;
...
procedure TNotaReturFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TNotaReturFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TNotaReturFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TNotaReturFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TNotaReturFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TNotaReturFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TNotaReturFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TNotaReturFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TNotaReturFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TNotaReturFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TNotaReturFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TNotaReturFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TNotaReturFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TNotaReturFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TNotaReturFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TNotaReturFrm.QLookLokasiBeforeOpen(DataSet: TDataSet);
begin
  QLookLokasi.SetVariable('kd_jns_item',QDetailKD_JNS_ITEM.AsString);
end;

procedure TNotaReturFrm.LookLokasiCloseUp(Sender: TObject;
...
procedure TNotaReturFrm.LookLokasiEnter(Sender: TObject);
begin
  QLookLokasi.Close;
  QLookLokasi.Open;
end;

procedure TNotaReturFrm.BtnFormulasiClick(Sender: TObject);
begin
  if ((vkd_jns_item='1') and (copy(wwDBEdit1.Text,1,3)<>'R01')) then
  begin
...
procedure TNotaReturFrm.QBrowseCalcFields(DataSet: TDataSet);
begin
  QBrowseQTY2.AsFloat:=QBrowseQTY.AsFloat/cball2kg;
end;

procedure TNotaReturFrm.QDetailCalcFields(DataSet: TDataSet);
begin
  QDetailQTY2.AsFloat:=QDetailQTY.AsFloat/cball2kg;
  QDetailSUB_TOTAL.AsFloat:=QDetailQTY.AsFloat*QDetailHARGA.AsFloat;
end;

procedure TNotaReturFrm.LookSuplierCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
var
  vkota : string;
begin
...
procedure TNotaReturFrm.LookSuplierEnter(Sender: TObject);
begin
  DMFrm.QSuplierBenang.Open;
end;

procedure TNotaReturFrm.LookSuplierUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
