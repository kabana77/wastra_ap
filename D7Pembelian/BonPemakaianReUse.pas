...
type
  TBonPemakaianWasteFrm = class(TForm)
    Panel1: TPanel;
...
var
  BonPemakaianWasteFrm: TBonPemakaianWasteFrm;
...
procedure TBonPemakaianWasteFrm.FormClose(Sender: TObject;
...
procedure TBonPemakaianWasteFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QLookSubBagian.Close;
  QLookItem.Close;
end;

procedure TBonPemakaianWasteFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TBonPemakaianWasteFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TBonPemakaianWasteFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TBonPemakaianWasteFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TBonPemakaianWasteFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
  QLookItem.Close;
...
procedure TBonPemakaianWasteFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TBonPemakaianWasteFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TBonPemakaianWasteFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TBonPemakaianWasteFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TBonPemakaianWasteFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TBonPemakaianWasteFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TBonPemakaianWasteFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TBonPemakaianWasteFrm.LookBagianCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
    QDetailID_SUB_BAG.AsString:=QLookSubBagianID_SUB_BAG.AsString;
...
procedure TBonPemakaianWasteFrm.LookItemEnter(Sender: TObject);
begin
  QLookItem.Open;
end;

procedure TBonPemakaianWasteFrm.QLookItemBeforeOpen(DataSet: TDataSet);
var
  vdd,vmm,vyy : word;
  vtgl_awal : TDate;
begin
...
procedure TBonPemakaianWasteFrm.LookBagianEnter(Sender: TObject);
begin
  QLookSubBagian.Open;
end;

procedure TBonPemakaianWasteFrm.LookItemCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TBonPemakaianWasteFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TBonPemakaianWasteFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TBonPemakaianWasteFrm.ECariChange(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TBonPemakaianWasteFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TBonPemakaianWasteFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TBonPemakaianWasteFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TBonPemakaianWasteFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TBonPemakaianWasteFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.QLookSubBagianBeforeOpen(DataSet: TDataSet);
begin
  QLookSubBagian.SetVariable('kd_bag',vkd_bag);
end;

procedure TBonPemakaianWasteFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TBonPemakaianWasteFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
...
procedure TBonPemakaianWasteFrm.FormCloseQuery(Sender: TObject;
...
procedure TBonPemakaianWasteFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TBonPemakaianWasteFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TBonPemakaianWasteFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TBonPemakaianWasteFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TBonPemakaianWasteFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TBonPemakaianWasteFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TBonPemakaianWasteFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TBonPemakaianWasteFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TBonPemakaianWasteFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TBonPemakaianWasteFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TBonPemakaianWasteFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TBonPemakaianWasteFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TBonPemakaianWasteFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TBonPemakaianWasteFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TBonPemakaianWasteFrm.LookItemUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TBonPemakaianWasteFrm.LookBagianUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookItem.Close;
  QLookItem.Open;
...
procedure TBonPemakaianWasteFrm.LookItemUserButton2Click(Sender: TObject;
  LookupTable: TDataSet);
begin
...
procedure TBonPemakaianWasteFrm.QDetailBeforePost(DataSet: TDataSet);
begin
...
procedure TBonPemakaianWasteFrm.LookMesinEnter(Sender: TObject);
begin
  QLookMesin.Open;
end;

procedure TBonPemakaianWasteFrm.LookMesinCloseUp(Sender: TObject;
...
procedure TBonPemakaianWasteFrm.LookMesinUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QLookMesin.Close;
  QLookMesin.Open;
...
procedure TBonPemakaianWasteFrm.vTglAWAL2Change(Sender: TObject);
begin
  vTglAkhir2.Date:=Trunc(EndOfTheMonth(VTglAwal2.Date));
end;

procedure TBonPemakaianWasteFrm.vTglAkhir2Change(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.vOperand2Click(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.cbTanggal2Click(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.BitBtn6Click(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.LookBudgetEnter(Sender: TObject);
begin
...
procedure TBonPemakaianWasteFrm.LookBudgetCloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
{MAA}
...
procedure TBonPemakaianWasteFrm.LookBagian2CloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  if modified then
    QDetailID_SUB_BAG.AsString:=QLookSubBagianID_SUB_BAG.AsString;
...
procedure TBonPemakaianWasteFrm.LookBagian2Enter(Sender: TObject);
begin
  QLookSubBagian.Open;
end;

procedure TBonPemakaianWasteFrm.LookItem2CloseUp(Sender: TObject;
  LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TBonPemakaianWasteFrm.LookItem2Enter(Sender: TObject);
begin
  QLookItem.Open;
end;

procedure TBonPemakaianWasteFrm.wwDBGrid4CalcCellColors(Sender: TObject;
  Field: TField; State: TGridDrawState; Highlight: Boolean; AFont: TFont;
  ABrush: TBrush);
begin
  if not Highlight then
...
procedure TBonPemakaianWasteFrm.wwDBGrid4DblClick(Sender: TObject);
var
  vnama_barang_org : String;
  vketerangan_org : String;
  vpos : Integer;
...
procedure TBonPemakaianWasteFrm.wwDBGrid4Enter(Sender: TObject);
begin
  if QTransaksi.State<>dsBrowse then
        try
          QTransaksi.Post;
...
procedure TBonPemakaianWasteFrm.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
...
procedure TBonPemakaianWasteFrm.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TBonPemakaianWasteFrm.QRBand5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
//  DetailBand2.ForceNewPage:=(vrecord_ke>Round(vRecord.Value)-1);
...
procedure TBonPemakaianWasteFrm.LookLOTEnter(Sender: TObject);
begin
  DMFrm.QNoLot.Close;
  DMFrm.QNoLot.Open;
end;
